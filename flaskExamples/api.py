...
# The default port for Flask webservice to run is 5000 in the development box
# (80 on production server)
...
class PersonalInfo(Resource):
    ...
    def __init__(self):
        ...
        # The first argument to the file is the class name which is db specific
    ...
    def get(self):
        ...
        query = " SELECT row_to_json(t) AS person"\
                ...
                "   ("\
                "   SELECT "\
                "   COALESCE(ARRAY_TO_JSON(ARRAY_AGG(ROW_TO_JSON(d))), '[]')"\
                "   FROM ("\
                "   SELECT  address_type,"\
                "            door,"\
                "            street,"\
